...
import { IconArrowLeft, IconReload, IconToggleRight, IconArrowNarrowRight, 
...
function Create() {
	let nav = useNavigate();
	const [form, setForm] = useState({ type: 'once' })
			...
			<div className="center grow">
				<div className="flex flex-col gap-y-2 p-2 justify-center w-[75%] h-content rounded-xl bg-neutral-800">
					<div className="flex gap-x-2 w-full">
						...
						<input type="text" className="rounded-md w-[15%] text-center bg-neutral-600 self-center h-[45px] p-1 text-[24px] opacity-80 " maxLength={1} autoCapitalize={'words'} placeholder='Key' 
							onKeyDown={e => {
								e.preventDefault();
					...
					<div className="flex w-full justify-center gap-x-5  ">
						<div className="rounded-md cursor-pointer p-2 bg-neutral-700 hover:bg-neutral-600 center w-[120px] aspect-[1/.4]  " onClick={e => nav('/')}>Cancel</div>
...
const Macro = ({ macro }) => {
	...
	return (
		<div className={` ${!active && '!bg-neutral-800 hover:!bg-neutral-700 '} border-b border-neutral-700 text-[25px] center h-[75px]`} onClick={e => {
			console.log(macro);
			setActive(!active)
			send('run', JSON.stringify(macro))
		}}>
			<div className="">{macro.name}</div>
		</div>
	)
}
...
function Home() {
	let nav = useNavigate()
	const [macros, setMacros] = useState(macros_bc.length ? macros_bc : [])

	useEffect(() => {
		if(macros_bc.length) return;
		send('load')
	...
	return (
		<>
			<div className="bg-neutral-950 h-[75px] hover:bg-neutral-700 center text-[25px] flex gap-2 " onClick={e => nav('/create')} >
				<IconSquareRoundedPlus size={24} /> Create New</div>
				<div className="overflow-scroll odd:!bg-teal-900 even:!bg-teal-800 ">
					{macros.map(e => <Macro macro={e}/>)}
				</div>
		</>
	)
}
