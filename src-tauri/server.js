...
let py = execFile(path.join(__dirname, "dist/kp.exe"));
// let py = spawn('py', [path.join(__dirname, "kp.py")]);
...
let macros = global.macros;
let sets = global.sets;
...
let ports = {
  run: (e) => {
    let macs = JSON.parse(e.pl);
    // macs = macs.map(e => global.macros[e])
    runMacro(e.pl);
    send("running", JSON.stringify({ running, paused }));
  },
  off: (e) => {
    let mac = JSON.parse(e.pl);
    if (running[mac.id]) {
      worker.postMessage({ keycode: mac.keycode, type: mac.type });
      running[mac.id] = 0;
      paused[mac.id] = 0;
